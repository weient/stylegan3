...
def loss_per(img, real, batch_size):
    sum_loss = 0
    for i in range(0,5):
        sub = torch.sub(real[i], img[i])
        num = torch.numel(sub)
        out = torch.sum(sub.abs())/num
        sum_loss += (out/batch_size) #除以batch_size
    return sum_loss/5 #除以feature map數量

def loss_text(img, real, batch_size):
    sum_loss = 0
    for i in range(0,5):
        sub = gram_matrix(real[i]) - gram_matrix(img[i])
        out = torch.sum(sub.abs())
        sum_loss += (out/batch_size)
    return sum_loss/5

def loss_emb(img, real, batch_size):
    sub = real[5]-img[5]
    out = torch.sum(sub.abs())
...
def call_type(in_img, in_real):
    ...
    per = loss_per(maps, maps_real, in_img.shape[0])
    tex = loss_text(maps, maps_real, in_img.shape[0])
    emb = loss_emb(maps, maps_real, in_img.shape[0])
    #print("per: ", per)
    #print("tex: ", tex)
    #print("emb: ", emb)
